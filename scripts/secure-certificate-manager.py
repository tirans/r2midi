...
class SecureCertificateManager:
    ...
    def setup_github_actions(self):
        ...
        self.cert_files["app"] = self.temp_dir / "app_cert.p12"
        self.cert_files["installer"] = self.temp_dir / "installer_cert.p12"
        ...
        self.signing_identity = self._extract_identity_from_p12(
            self.cert_files["app"], "Application"
        )
        self.installer_identity = self._extract_identity_from_p12(
    ...
    def setup_local(self, config_file_path):
        ...
        try:
            ...
            p12_path = Path(apple_dev["p12_path"])
            password = apple_dev["p12_password"]
            
            # Check if certificates exist
            app_cert = p12_path / "app_cert.p12"
            installer_cert = p12_path / "installer_cert.p12"
            
            if not app_cert.exists() or not installer_cert.exists():
                log_error("Certificate files not found in p12_path")
                return False
    ...
    def _extract_identity_from_p12(self, p12_file, cert_type):
        """Extract certificate identity from P12 file without importing to keychain"""
        try:
            password = os.getenv("APPLE_CERT_PASSWORD")
            if not password:
                # Try to get from config if local
                config_file = Path("apple_credentials/config/app_config.json")
            ...
            # Parse the certificate to extract the subject
            cert_pem = result.stdout
            
            # Extract subject with another openssl command
            cmd = ["openssl", "x509", "-subject", "-noout"]
            result = subprocess.run(cmd, input=cert_pem, capture_output=True, text=True)
            
            if result.returncode != 0:
                log_error(f"Failed to parse {cert_type} certificate subject")
                return None
...
<dict>
    ...
    <true/>
        ...
        try:
            # Sign using codesign with direct certificate file reference
            password = os.getenv("APPLE_CERT_PASSWORD")
            if not password:
                config_file = Path("apple_credentials/config/app_config.json")
                if config_file.exists():
            ...
            # Use security command to temporarily unlock certificate for signing
            # This avoids keychain prompts by using the certificate directly
            cmd = [
