#!/bin/bash
# sign-and-notarize.sh - Main signing and notarization orchestrator
...
REQUIRED_SCRIPTS=("app-cleaner.sh" "codesign.sh")
if [ "$SKIP_NOTARIZATION" = false ]; then
    REQUIRED_SCRIPTS+=("notarize.sh")
fi
...
for target in "${TARGETS[@]}"; do
    ...
    if [[ "$target" == *.app ]]; then
        ...
        if "$SCRIPT_DIR/app-cleaner.sh" "$target"; then
    ...
    echo "  🔐 Code signing..."
    if [[ "$target" == *.app ]]; then
        if "$SCRIPT_DIR/codesign.sh" "$target" "$CERT_IDENTITY" "$ENTITLEMENTS_FILE"; then
    ...
    if [ "$TARGET_SUCCESS" = true ] && [ "$SKIP_NOTARIZATION" = false ]; then
        echo "  📤 Notarizing..."
        if "$SCRIPT_DIR/notarize.sh" "$target"; then
