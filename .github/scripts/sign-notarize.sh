...
sign_target_enhanced() {
    ...
    if [[ "$target" == *.app ]]; then
        ...
        find "$target" -name "*.dylib" -type f | while read -r dylib; do
            ...
            if ! codesign --force --sign "$identity" --timestamp --options runtime "$dylib" 2>/dev/null; then
        ...
        find "$target" -name "*.so" -type f | while read -r so_file; do
            ...
            if ! codesign --force --sign "$identity" --timestamp --options runtime "$so_file" 2>/dev/null; then
        ...
        if [ -d "$target/Contents/Frameworks" ]; then
            find "$target/Contents/Frameworks" -name "*.framework" -type d | while read -r framework; do
                ...
                if ! codesign --force --sign "$identity" --timestamp --options runtime "$framework" 2>/dev/null; then
        ...
        if [ -f "$target/Contents/MacOS/"* ]; then
            local main_executable=$(find "$target/Contents/MacOS" -type f -perm +111 | head -1)
            if [ -n "$main_executable" ]; then
                log_info "Signing main executable: $(basename "$main_executable")"
                if ! codesign --force --sign "$identity" --timestamp --options runtime --entitlements "$entitlements" "$main_executable"; then
        ...
        if codesign --force --sign "$identity" --timestamp --options runtime --entitlements "$entitlements" --deep "$target"; then
...
select_signing_identity() {
    ...
    log_info "Selecting signing identity for: $cert_type"

    local identity=""

    # Try to find identity with team ID if provided
    if [ -n "$team_id" ]; then
        identity=$(security find-identity -v -p codesigning | grep "$cert_type" | grep "$team_id" | head -1 | sed 's/.*) \(.*\)/\1/' | tr -d '"' || true)
    fi

    # Fallback to first available identity
    if [ -z "$identity" ]; then
        identity=$(security find-identity -v -p codesigning | grep "$cert_type" | head -1 | sed 's/.*) \(.*\)/\1/' | tr -d '"' || true)
    fi

    if [ -n "$identity" ]; then
        log_info "Selected identity: $identity"
        echo "$identity"
        return 0
    else
        log_error "No suitable signing identity found"
        return 1
    fi
}
...
# Function to sign a target (app bundle or pkg file)
sign_target_enhanced() {
    local target="$1"
    local identity="$2"
    local entitlements="$3"

    log_info "Signing target: $(basename "$target")"
    log_info "Target path: $target"
    log_info "Identity: $identity"

    if [[ "$target" == *.app ]]; then
        # Handle .app bundle signing
        log_info "Signing .app bundle..."

        # Clean up app bundle to remove resource forks and metadata
        log_info "Performing deep clean of app bundle..."

        # Use bulletproof clean if available
        local repo_root="${SCRIPT_DIR}/../.."
        local bulletproof_script="${repo_root}/scripts/bulletproof_clean_app_bundle.py"
        ...
        if [ -f "$bulletproof_script" ]; then
            ...
            if python3 "$bulletproof_script" --method ditto "$target"; then
                ...
                log_warning "Bulletproof clean failed, using standard cleanup"
            fi
        else
            log_info "Using standard cleanup..."
            find "$target" -name ".DS_Store" -delete 2>/dev/null || true
            find "$target" -name "__MACOSX" -type d -exec rm -rf {} + 2>/dev/null || true
            find "$target" -name "._*" -delete 2>/dev/null || true
            xattr -rc "$target" 2>/dev/null || true
        ...
        log_error "Unsupported target type: $target"
        return 1
    fi
...
# Function to notarize a target (app bundle or pkg file)
notarize_target_enhanced() {
    local target="$1"

    log_info "Notarizing target: $(basename "$target")"
    log_info "Target path: $target"
...
find_and_process_targets() {
    ...
    if [ -d "artifacts" ]; then
        while IFS= read -r -d '' pkg_path; do
            if [ -f "$pkg_path" ]; then
                log_info "Found package: $pkg_path"
    ...
    while IFS= read -r -d '' pkg_path; do
        if [ -f "$pkg_path" ]; then
            log_info "Found package in build dir: $pkg_path"
            found_targets+=("$pkg_path")
        fi
    ...
    log_info "Found ${#found_targets[@]} target(s) for signing"
